// In this, change theme and color. It should not look like existing game:
...
const Game = () => {
  ...
  const pinkAudioRef = useRef(null);
  ...
  const playSound = (player) => {
    if (player === 'Pink') {
      pinkAudioRef.current.play();
    } else {
      blueAudioRef.current.play();
    }
  };

  async function onPinkWin(){
    console.log("Pink wins! Perform custom action here.");
        let provider = window.ethereum;
    const web3 = new Web3(provider);
    let accounts = await web3.eth.getAccounts();
    contract = new web3.eth.Contract(abi, ADDRESS);
  ...
  const handleClick = (col) => {
    if (winner || currentPlayer === 'Blue') return;
  ...
  const findBestMove = (board) => {
    ...
    for (let col = 0; col < COLS; col++) {
      ...
      if (row >= 0) {
        newBoard[row][col] = 'Pink';
        if (checkWin(newBoard) === 'Pink') {
          return col;
        }
      }
    }
  ...
  return (
    <header className="App-header">
      ...
      <h1 className="game-title">Disc Drop</h1>
          {selected !== null ? (
    ...
    <div className="board-container">
      ...
      {winner && (
      <h2 style={{ color: winner === 'Pink' ? 'pink' : 'blue' }}>{winner} wins!</h2>)}
      <audio ref={pinkAudioRef} src="sound/coin_drop.mp3" />
      <audio ref={blueAudioRef} src="sound/coin_drop.mp3" />
    </div>
    )}
    </header>
  );
